...
struct DailyInsightEngine {
    ...
    private static func buildContext(
        ...
        for week in plan.weeks {
            for day in week.days {
                ...
                if d == today {
                    ...
                    ctx.currentPhase   = week.phase   // TrainingPhase enum
                ...
                if d == cal.startOfDay(for: tomorrow) {
                    ctx.tomorrowDay       = day
                    ctx.tomorrowIsLongRun = day.workoutType.contains("Long Run")
                }
            }
        }
        ...
        // Taper / peak — use enum comparison, no string matching
        let weeksToRace = ctx.totalWeeks - ctx.currentWeekNum
        ctx.isInTaper   = weeksToRace <= 2 && ctx.currentPhase == .taper
        ctx.isPeakWeek  = ctx.currentPhase == .peak
